...
`#defined`d constants.  Access from Haskell to such entities is the
domain of language-specific tools, which provide added convenience over the plain FFI as defined here.
...
pointers.  If neither of the specifiers `static`, `dynamic`, or `wrapper` is given, `static` is assumed.  The specifier
...
/ Static addresses:  The type of an imported address is constrained to be
  of the form `Ptr` $a$ or `FunPtr` $a$, where
  $a$ can be any type.
...
if no function prototype is in scope, _default argument promotion_ is
...
_after_ argument promotion.  For example, consider the following C
...
receive a value of type `double`, _not_ `float`.  Hence, the
...
#figure(
  ...
  table(
    ...
    table.vline(x: 3),
    table.hline(),
    table.header([C symbol], [Haskell symbol],[Constraint on concrete C type]),
    table.hline(),
...
#figure(
  ...
  table(
    columns: 3,
    align: (left, left, left),
    table.header([CPP symbol], [Haskell value], [Description]),
    ...
    [`HS_FLOAT_ROUND`],[n/a],[rounding style as per @C99],
    [`HS_FLOAT_EPSILON`],[n/a],[difference between 1 and the least value greater than 1 as per @C99],
    [`HS_DOUBLE_EPSILON`],[n/a],[(as above)],
    [`HS_FLOAT_DIG`],[n/a],[number of decimal digits as per @C99],
    [`HS_DOUBLE_DIG`],[n/a],[(as above)],
    [`HS_FLOAT_MANT_DIG`],[`floatDigits :: Float`],[],
    [`HS_DOUBLE_MANT_DIG`],[`floatDigits :: Double`],[],
    [`HS_FLOAT_MIN`],[n/a],[minimum floating point number as per @C99],
    [`HS_DOUBLE_MIN`],[n/a],[(as above)],
    [`HS_FLOAT_MIN_EXP`],[`fst . floatRange :: Float`],[],
    [`HS_DOUBLE_MIN_EXP`],[`fst . floatRange :: Double`],[],
    [`HS_FLOAT_MIN_10_EXP`],[n/a],[minimum decimal exponent as per @C99],
    [`HS_DOUBLE_MIN_10_EXP`],[n/a],[(as above)],
    [`HS_FLOAT_MAX`],[n/a],[maximum floating point number as per @C99],
    [`HS_DOUBLE_MAX`],[n/a],[(as above)],
    [`HS_FLOAT_MAX_EXP`],[`snd . floatRange :: Float`],[],
    [`HS_DOUBLE_MAX_EXP`],[`snd . floatRange :: Double`],[],
    [`HS_FLOAT_MAX_10_EXP`],[n/a],[maximum decimal exponent as per @C99],
