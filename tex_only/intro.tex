...
\section{Program Structure}\index{program structure}
\label{programs}
...
\mbox{$\it \makebox{\tt if}\ e_1\ \makebox{\tt then}\ e_2\ \makebox{\tt else}\ e_3$}.  Generally the italicized names are
mnemonic, such as \mbox{$\it e$} for expressions, \mbox{$\it d$} for
declarations, \mbox{$\it t$} for types, etc.

\section{The \Haskell{} Kernel}
