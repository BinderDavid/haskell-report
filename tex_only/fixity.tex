...
The function \mbox{\tt resolve} takes a list in which the elements are
expressions or operators, i.e. an instance of the \mbox{$\it infixexp$}
non-terminal in the context-free grammar.  It returns either \mbox{\tt Just\ e}
where \mbox{\tt e} is the resolved expression, or \mbox{\tt Nothing} if the input does
...
the purposes of producing a useful error message, but the \mbox{\tt Maybe} type
...
(the caller holds E0).  The job of \mbox{\tt parse} is to build the expression
to the right of \mbox{\tt op1}, returning the expression and any remaining
input.
...
\item if \mbox{\tt op1} and \mbox{\tt op2} have the same precedence, but they do not
  have the same associativity, or they are declared to be nonfix, then
  the expression is illegal.

\item If \mbox{\tt op1} has a higher precedence than \mbox{\tt op2}, or \mbox{\tt op1} and \mbox{\tt op2}
  should left-associate, then we know that the expression to the right
  of \mbox{\tt op1} is \mbox{\tt E1}, so we return this to the caller.

\item Otherwise, we know we want to build an expression of the form
  \mbox{\tt E1\ `op2`\ R}.  To find \mbox{\tt R}, we call \mbox{\tt parseNeg\ op2\ tokens} to compute
  the expression to the right of \mbox{\tt op2}, namely \mbox{\tt R} (more about
  \mbox{\tt parseNeg} below, but essentially if \mbox{\tt tokens} is of the form \mbox{\tt (E2\ :\ rest)},
  then this is equivalent to \mbox{\tt parse\ op2\ E2\ rest}).  Now, we
  have
\[
      \mbox{\tt E0\ `op1`\ (E1\ `op2`\ R)\ `op3`\ ...}
\]
  where \mbox{\tt op3} is the next operator in the input.  This is an instance of
  (1) above, so to continue we call parse, with the new \mbox{\tt E1\ ==\ (E1\ `op2`\ R)}.
\end{enumerate}

To initialise the algorithm, we set \mbox{\tt op1} to be an imaginary operator
with precedence lower than anything else.  Hence \mbox{\tt parse} will consume
the whole input, and return the resulting expression.

The handling of the prefix negation operator, \mbox{\tt -}, complicates matters
...
the left of \mbox{\tt -}, if there is one, must have precedence lower than 6
for the expression to be legal.  The negation operator itself may
left-associate with operators of the same fixity (e.g. \mbox{\tt +}).  So for
example \mbox{\tt -a\ +\ b} is legal and resolves as \mbox{\tt (-a)\ +\ b}, but \mbox{\tt a\ +\ -b} is
illegal.

The function \mbox{\tt parseNeg} handles prefix negation.  If we encounter a
...
way to case (3) above: compute the argument to \mbox{\tt -} by recursively
calling \mbox{\tt parseNeg}, and then continue by calling \mbox{\tt parse}.
